...
//		}
		...
		public void train (List <List<double>> trainingSamples, double learningRate, LearningAlgorithm learningAlgorithm)
		{
			for (int i = 0; i < trainingSamples.Count; ++i) {
				List <double> output = this.feedforward (trainingSamples [i]);
				this.network = learningAlgorithm.learn (learningRate, trainingSamples [i], output, this.network);
			}
		}
	}
}
